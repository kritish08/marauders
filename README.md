...
| **Gitansh Kapoor** ([@GitanshKapoor](https://github.com/GitanshKapoor)) | Led the augmented reality (AR) development, contributed to frontend implementation, engineered the integration layer between the frontend and backend for differnce features and implementation, developed and optimized ELT pipelines for data preparation and ML model training, optimized the application for efficient on-device memory and compute utilization, collaborated on cross-platform feature integration, and contributed to the application's overall user experience and project workflow. |
